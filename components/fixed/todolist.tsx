...
const IconGroup = ({note, post, file, equipment}:IconGroupProps):JSX.Element => {
    return(
        <Center>  
            { note && note.date && <IconItem label={note.date} children={<Calendar size={27}/>}/>}
            { note && note.location && <IconItem label={note.location} children={<MapPin size={27}/>}/>}
            { file && file.type === 'image' && <IconItem label={'preview iamge'} fileType={file.type} href={file.blob} children={<Image size={27}/>}/>} 
            { file && file.type === 'file' && <IconItem label={'download file'} fileType={file.type} href={file.blob} children={<File size={27}/>}/>}
            
            { post && (
                post.type === 'post' ? <IconItem label={post.title} href={post.url} fileType={'link'} children={<GlobeHemisphereWest size={27}/>}/>
                : post.type === 'photo' ? <IconItem label={post.title} href={post.url} fileType={'link'} children={<Image size={27}/>}/>
                : post.type === 'collection' ? <IconItem label={post.title} href={post.url} fileType={'link'} children={<BookmarkSimple size={27}/>}/>
                : <IconItem label={'unknown'} children={<Question size={27}/>}/>
            )}

            {equipment && equipment.camera && <IconItem label={equipment.camera} children={<Camera size={27}/>}/>}
            {equipment && equipment.lens && <IconItem label={equipment.lens} children={<Cylinder size={27}/>}/>}
            {equipment && equipment.other && <IconItem label={equipment.other} children={<Lightning size={27}/>}/>}
        </Center>
    )
}
